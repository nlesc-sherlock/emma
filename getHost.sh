...
ssh-keygen -f "/root/.ssh/known_hosts" -R [127.0.0.1]:2202 2> /tmp/err 1>/tmp/out
sed -i 's/## vagrant-hostmanager-start.*//g' /etc/hosts
sed -i 's/.*'"$HOST_NAME"'.*//g' /etc/hosts
sed -i 's/## vagrant-hostmanager-end.*//g' /etc/hosts
sed -i '/^\s*$/d' /etc/hosts
ssh -i ${KEYS_LOCATION}/$host_name/private_key ubuntu@127.0.0.1 -o StrictHostKeyChecking=no -p 2202 "tail -n 13 /etc/hosts" >> /etc/hosts
